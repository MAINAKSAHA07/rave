...
router.post('/scan', async (req, res, next) => {
  try {
    ...
    if (!isValidTicketCode(ticketCode)) {
      res.status(400).json({ error: 'Invalid ticket code format' });
      return;
    }
    ...
    if (ticket.event_id !== eventId) {
      res.status(400).json({ error: 'Ticket does not belong to this event' });
      return;
    }
    ...
    if (ticket.status === 'checked_in') {
      res.status(400).json({ error: 'Ticket already checked in', alreadyCheckedIn: true });
      return;
    }
