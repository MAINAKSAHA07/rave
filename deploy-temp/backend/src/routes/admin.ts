...
const requireSuperAdmin = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  try {
    ...
    if (!user || user.role !== 'super_admin') {
      res.status(403).json({ error: 'Requires Super Admin role' });
      return;
    }
...
const requireAdminOrOrganizerStaff = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  try {
    ...
    if (!isAdmin && !hasBackofficeAccess) {
      res.status(403).json({ error: 'Requires Admin role or backoffice access' });
      return;
    }
  ...
  try {
    ...
    if (user.role !== 'admin' && user.role !== 'super_admin') {
      ...
      try {
        ...
        res.status(403).json({ error: 'You do not have permission to check in tickets for this event' });
        return;
      }
    }
      ...
      try {
        ...
        if (refundAmount > 0 && refundAmount <= availableRefund) {
          ...
          if (paymentMethod === 'cash' || !order.razorpay_payment_id) {
            ...
            await pb.collection('refunds').create(refundData);
...
router.get('/stats', requireAdmin, async (_req, res, next) => {
  try {
    ...
    const totalRefunds = orders.reduce((sum: number, order: any) => sum + (order.refunded_amount_minor || 0), 0);
    ...
    if (typeof can_manage_roles === 'boolean') {
      ...
      if (can_manage_roles) {
        ...
        if (!user.backoffice_access) {
          res.status(400).json({ error: 'User must have backoffice access to manage roles' });
          return;
        }
      }
