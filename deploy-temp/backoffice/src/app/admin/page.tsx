...
export default function AdminPage() {
  ...
  async function handleForceCancel() {
    ...
    try {
      ...
      setActionMessage(`Event cancelled successfully. Refunded: ${response.data.refunded} orders, Failed: ${response.data.failed} orders`);
      setEventIdToCancel('');
      setCancelReason('');
    } catch (error: any) {
      setActionMessage(`Error: ${error.response?.data?.error || error.message || 'Failed to cancel event'}`);
    } finally {
  ...
  async function handleForceRefund() {
    ...
    try {
      ...
      setActionMessage(`Error: ${error.response?.data?.error || error.message || 'Failed to refund order'}`);
    } finally {
      setActionLoading(false);
    }
  }
  ...
  async function handleApprove(applicationId: string) {
    try {
      ...
      if (error.response?.status === 403) {
        ...
        // Interceptor should have handled this, but just in case
        alert('Session expired. Please log in again.');
      } else {
        alert(`Error: ${error.response?.data?.error || error.message || 'Failed to approve application'}`);
      }
    }
  }
  ...
  async function handleReject(applicationId: string) {
    ...
    try {
      ...
      if (error.response?.status === 403) {
        ...
        alert(`Error: ${error.response?.data?.error || error.message || 'Failed to reject application'}`);
      }
    }
  }
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <h3 className="font-semibold">Management Pages</h3>
                <div className="space-y-1">
                  <Link href="/admin/events" className="text-blue-600 hover:underline block">
                    → Events Management
                  </Link>
                  <Link href="/admin/orders" className="text-blue-600 hover:underline block">
                    → Orders Management
                  </Link>
                  <Link href="/admin/organizers" className="text-blue-600 hover:underline block">
                    → Organizers Management
                  </Link>
                  <Link href="/admin/refunds" className="text-blue-600 hover:underline block">
                    → Refunds Management
                  </Link>
                  {user.role === 'super_admin' && (
                    <Link href="/admin/users" className="text-blue-600 hover:underline block font-semibold">
                      → User Management
                    </Link>
                  )}
                </div>
              ...
              <div className="space-y-2">
                <h3 className="font-semibold">Reports & Settings</h3>
                <div className="space-y-1">
                  <Link href="/admin/stats" className="text-blue-600 hover:underline block">
                    → Platform Statistics
                  </Link>
                </div>
              </div>
