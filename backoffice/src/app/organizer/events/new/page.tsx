...
export default function CreateEventPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // Super Admin or Admin: Allow creating events
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      // Super Admin or Admin: Need to get organizer from venue
      if (user.role === 'super_admin' || user.role === 'admin') {
        const venue = await pb.collection('venues').getOne(values.venue_id);
        organizerId = venue.organizer_id;
      } else {
      ...
      router.push(`/organizer/events/${record.id}`);
    } catch (error: any) {
          ...
          <CardContent>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-sm text-gray-500">Max size: 5MB. Formats: JPEG, PNG, WebP</p>
                {coverImage && (
                  <div className="mt-2">
                    <p className="text-sm text-green-600">Selected: {coverImage.name}</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-sm text-gray-500">You can select multiple images. Max size per image: 5MB. Formats: JPEG, PNG, WebP</p>
                {images.length > 0 && (
